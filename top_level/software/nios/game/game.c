...
static const uint8_t shot_max_y_relative_pos = 25;
...
static void erase_object(Object *object)
{
	if (object->is_alive)
	{
		erase_draw(object->x_coordinate, object->y_coordinate, object->x_size, object->y_size);
	}
}
	...
	for(uint8_t i = 0; i < number_of_objects; i++)
	{
		...
		switch ((objects + i)->type)
		{
			...
			case player_shot_type:
				if((objects + i)->is_alive)
				{
					if ((objects + i)->relative_pos < shot_max_y_relative_pos)
					{
						(objects + i)->y_coordinate += (objects + i)->velocity;
						(objects + i)->relative_pos +=1;
						draw_shot((objects + i)->x_coordinate, (objects + i)->y_coordinate);
					}
					else
					{
...
static void shoot(Object* shooter, Object* possible_shots, uint8_t number_of_possible_shots)
{
	for(uint8_t i = 0; i < number_of_possible_shots; i++)
	{
		if (!(possible_shots + i)->is_alive)
		{
			...
			(possible_shots + i)->y_coordinate = shooter->y_coordinate - (possible_shots + i)->y_size;
			(possible_shots + i)->relative_pos = 0;
			draw_shot((possible_shots + i)->x_coordinate, (possible_shots + i)->y_coordinate);
			break;
		}
	}
}
...
//		if ((shots + i)->is_alive)
//		{
//			IOWR_ALTERA_AVALON_UART_TXDATA(UART_BASE, 's');
//			IOWR_ALTERA_AVALON_PIO_DATA(PIO_0_BASE, IORD_ALTERA_AVALON_PIO_DATA(PIO_0_BASE) ^ (1 << 0));
//			for (uint8_t j = 0; j < number_of_targets; j++)
//			{
//				if ((targets + j)->is_alive)
//				{
//					IOWR_ALTERA_AVALON_UART_TXDATA(UART_BASE, 't');
...
//							{
//								IOWR_ALTERA_AVALON_UART_TXDATA(UART_BASE, 'y');
//								if (((shots + i)->x_coordinate < (targets + j)->x_coordinate + (targets + j)->x_size) ||
//									((shots + i)->x_coordinate > (targets + j)->x_coordinate - (shots +i)->x_size) )
//								{
////									(shots + i)->is_alive = 0x00;
////									(targets + j)->is_alive = 0x00;
//									IOWR_ALTERA_AVALON_UART_TXDATA(UART_BASE, j);
//
//			//							erese_object();
//								}
//							}
			...
			if ((shots + i)->is_alive && ((targets + j)->is_alive))
			{
				switch ((targets + j)->type) {
					case alien_type:
						...
						{
							IOWR_ALTERA_AVALON_UART_TXDATA(UART_BASE, 'y');
//							if ((((shots + i)->x_coordinate < ((targets + j)->x_coordinate) + (targets + j)->x_size)))// || ((shots + i)->x_coordinate > ((targets + j)->x_coordinate - (shots + i)->x_size)))
//							{
...
//							}
							...
							else
							{
								...
								{
									IOWR_ALTERA_AVALON_UART_TXDATA(UART_BASE, 'l');
									erase_object(shots + i);
									erase_object(targets + j);
									(shots + i)->is_alive = 0x00;
									(targets + j)->is_alive = 0x00;
